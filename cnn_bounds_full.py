...
from tensorflow.contrib.keras.api.keras.layers import Conv2D, MaxPooling2D, AveragePooling2D, InputLayer, BatchNormalization, Reshape, GlobalAveragePooling1D, Conv1D
    ...
    def __init__(self, model, inp_shape = (2048,3)):
        ...
        while i < len(model.layers):
            ...
            if type(layer) == Conv1D:
                ...
                print('reshape')
                '''
                cur_shape = (int(np.sqrt(cur_shape[0])),int(np.sqrt(cur_shape[0])))
...
def UL_conv_bound(A, B, pad, stride, shape, W, b, inner_pad, inner_stride, inner_shape):
    ...
    if A.shape[3] != W.shape[0]:
        A = A.repeat(A.shape[3]).reshape((A.shape[0],A.shape[1],A.shape[2],W.shape[0]))
...
def UL_relu_bound(A, B, pad, stride, alpha_u, alpha_l, beta_u, beta_l):
    ...
    B_new = B.copy()
    #print('alpha_u',alpha_u.shape)
    for x in range(A_new.shape[0]):
        
        p_start = np.maximum(0, 0-x)
        #print('p_start',p_start);,m
        ...
        for z in range(A_new.shape[1]):
            for p in range(p_start, p_end):
                    #for q in range(q_start, q_end):
                    for r in range(A.shape[3]):
                            
                            A_new[x,z,p,r] += A_plus[x,z,p,r]*alpha_u[p+x,r]
                            A_new[x,z,p,r] += A_minus[x,z,p,r]*alpha_l[p+x,r]
                            B_new[x,z] += A_plus[x,z,p,r]*beta_u[p+x,r]
                            B_new[x,z] += A_minus[x,z,p,r]*beta_l[p+x,r]
    return A_new, B_new
...
def compute_bounds(weights, biases, out_shape, nlayer, x0, eps, p_n, pads, strides, sizes, types, LBs, UBs):
    if types[nlayer-1] == 'relu':
        ...
        '''
            ...
            print('reshape')
            weights[nlayer-1]
            A_u = weights[nlayer-1].reshape((1, weights[nlayer-1].shape[0], weights[nlayer-1].shape[1], weights[nlayer-1].shape[2]))*np.ones((out_shape[0], weights[nlayer-1].shape[0], weights[nlayer-1].shape[1], int(weights[nlayer-1].shape[2]**2)), dtype=np.float32)
